...
fn settings_path(data_dir: &Path) -> Result<PathBuf, String> {
    std::fs::create_dir_all(data_dir).map_err(|e| e.to_string())?;
    Ok(data_dir.join("network_settings.json"))
}

pub fn load_network_config(data_dir: &Path) -> NetworkConfigSnapshot {
    let path = match settings_path(data_dir) {
        Ok(p) => p,
        Err(_) => {
