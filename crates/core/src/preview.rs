...
pub async fn cmd_get_preview(
    ...
    let cache_dir = cache_dir.join("previews");
...
pub async fn cmd_clean_preview_cache(cache_dir: &std::path::Path) -> Result<(), String> {
    let cache_dir = cache_dir.join("previews");
...
pub async fn cmd_clean_cache(cache_dir: &std::path::Path, thumb_dir: &std::path::Path) -> Result<(), String> {
    let cache_dir = cache_dir.join("previews");
    let thumb_dir = thumb_dir.join("thumbnails");
...
pub async fn cmd_get_thumbnail(
    ...
    let cache_dir = thumb_dir.join("thumbnails");
    if tokio::fs::metadata(&cache_dir).await.is_err() {
        let _ = tokio::fs::create_dir_all(&cache_dir).await;
    }
