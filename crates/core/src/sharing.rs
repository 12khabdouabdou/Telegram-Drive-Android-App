...
pub fn create_share(
    ...
    let link = if share_host == "http://localhost" {
        format!("http://localhost:{}/d/{}", stream_port, token)
    } else {
...
pub fn list_shares(conn: &sqlite::Connection, stream_port: u16) -> Result<Vec<ShareInfo>, String> {
    let mut stmt = conn
        .prepare(
    ...
    while let sqlite::State::Row = stmt.next().map_err(|e| e.to_string())? {
        ...
        let created_at = stmt
            ...
            .map_err(|e| e.to_string())?;
        let link = format!("http://localhost:{}/d/{}", stream_port, id);
...
pub fn revoke_share(conn: &sqlite::Connection, id: String) -> Result<(), String> {
    let mut stmt = conn
